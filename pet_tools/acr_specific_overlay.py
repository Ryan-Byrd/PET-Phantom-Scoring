...
def _ensure_ct_toolkit_on_path():
    ...
    for path in (ct_toolkit_path, repo_root):
...
FOV_MM = 250.0                 # REQUIRED: 250x250 mm (updated)
...
def load_config(path: str | None) -> dict:
    ...
    try:
        ...
        raise RuntimeError(f"Failed to read config '{path}': {exc}")


def setup_debugging(enabled: bool, debug_dir: str | None):
    if not enabled:
        return
...
def main(argv=None) -> int:
    ...
    parser.add_argument("--debug-dir", help="Optional directory to store debug artifacts")
    args = parser.parse_args(argv)

    try:
        config = load_config(args.config)
        debug_enabled = args.debug or bool(config.get("debug"))
        debug_dir = args.debug_dir or config.get("debug_dir")
        setup_debugging(debug_enabled, debug_dir)

        tkinter_ok = _has_tkinter()

        in_dir = args.input_dir or config.get("input_dir")
        if in_dir:
            in_dir = os.path.abspath(in_dir)
        if not in_dir:
            if not tkinter_ok:
                raise RuntimeError("tkinter is not available; pass --input and --output to run headless from dialogs.")
            in_dir = ask_folder("Select PET DICOM Folder (INPUT)")
        if not in_dir:
            INFO("Cancelled: no input folder.")
            return 1

        out_dir = args.output_dir or config.get("output_dir")
        if out_dir:
            out_dir = os.path.abspath(out_dir)
        if not out_dir:
            if not tkinter_ok:
                raise RuntimeError("tkinter is not available; pass --output to select output folder without dialogs.")
        ...
        use_auto = args.auto or bool(config.get("auto"))
        use_manual = args.manual or bool(config.get("manual"))
        if use_auto and use_manual:
            raise RuntimeError("Choose only one of --auto or --manual")
        if use_manual and not tkinter_ok:
            raise RuntimeError("Manual selection requires tkinter, which is unavailable in this environment.")

        if not use_auto and not use_manual:
            if not tkinter_ok:
        ...
        ipp = getattr(ds, "ImagePositionPatient", None)
        if ipp and len(ipp) >= 3:
            z_pos_mm = float(ipp[2])
            z_text = f"Z-axis Position {z_pos_mm:.2f} mm"
        else:
            z_text = "Z N/A"
