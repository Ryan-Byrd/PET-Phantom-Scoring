...
def main(argv=None):
    ...
    parser = argparse.ArgumentParser(
        description="Manual PET phantom scoring: user picks slices, centers, and ROIs."
    )
    parser.add_argument("--input", "-i", help="Input PET DICOM folder (optional; else dialog)")
    parser.add_argument("--output", "-o", help="Output PARENT folder; a subfolder name is still prompted (optional; else dialogs)")
    ...
    try:
        ...
        if args.output:
            ...
            name = simpledialog.askstring("Output Subfolder", "Enter a name for the output subfolder:")
            if not name:
                INFO("Cancelled: no output subfolder name."); return
            out_dir = os.path.join(base, name)
            os.makedirs(out_dir, exist_ok=True)
        else:
        ...
        if args.debug:
            print(f"[debug] input  : {in_dir}")
            print(f"[debug] output : {out_dir}")
            if metadata_overrides is not None:
                print(f"[debug] metadata overrides: {metadata_overrides.source_path}")
        ...
        uni_idx, suv_idx = select_four(disp_imgs, (row_mm, col_mm))
        ...
        # --- Automatic or manual phantom center detection ---
        try:
            modality = getattr(dsets[0], "Modality", "PT").upper()
            pixel_spacing = np.mean([float(x) for x in dsets[0].PixelSpacing])
            mid_idx = len(dsets) // 2
            ...
            cy, cx, radius_px = find_phantom_center_generalized(
                px_array, pixel_size_mm=pixel_spacing, modality=modality, debug=True
            )
            auto_center = np.array([cx, cy])
            ...
            ax.imshow(px_array, cmap="gray")
            ax.plot(auto_center[0], auto_center[1], "ro", ms=10)
            ax.set_title("Press ENTER to keep this center, BACKSPACE to redefine manually")

            center_accepted = {"value": None}

            def on_key(event):
                if event.key == "enter":
                    center_accepted["value"] = True
                    plt.close(fig)
                elif event.key == "backspace":
                    center_accepted["value"] = False
                    plt.close(fig)

            fig.canvas.mpl_connect("key_press_event", on_key)
            plt.show()

            if not center_accepted["value"]:
                print("Select new center manually (click once)...")
                fig2, ax2 = plt.subplots(figsize=(7, 7))
                ax2.imshow(px_array, cmap="gray")
                ax2.set_title("Click to define new phantom center")
            ...
            fig, ax = plt.subplots(figsize=(7, 7))
            ax.imshow(uni_imgs_disp[0], cmap="gray")
            ax.set_title("Automatic detection failed. Click to define center manually.")
            pts = plt.ginput(1, timeout=0)
            plt.close(fig)
            if pts:
                center_uni_mm = np.array(pts[0]) * np.array([col_mm, row_mm])
            else:
                center_uni_mm = np.array([
                    uni_imgs_disp[0].shape[1] * col_mm / 2,
                    uni_imgs_disp[0].shape[0] * row_mm / 2
                ])
        ...
        # --- SUV ROI section ---
        C_mm, P25_mm, P16_mm = click_center_25_16(suv_img_disp, (row_mm, col_mm), dsets)
        ...
        INFO(f"Saved SUV ROI CSV → {out_csv}")

        # --- Append Uniformity statistics ---
        ...
        INFO(f"Appended Uniformity stats → {out_csv}")
        ...
        INFO(f"Saved overlay PNG → {png_overlay}")
        ...
        plt.show()
        INFO(f"Saved composite PNG → {composite_path}")
        INFO("Done.")
