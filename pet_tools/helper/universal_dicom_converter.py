...
def convert_all(input_dir, output_dir, workers=1):
    ...
    Uses asyncio + ProcessPoolExecutor for non-blocking I/O.
    """
    return asyncio.run(_convert_all_async(input_dir, output_dir, workers))


async def _convert_all_async(input_dir, output_dir, workers=1):
    """
    Async version of convert_all using ProcessPoolExecutor.
    Handles file operations and subprocess calls without blocking the event loop.
    """
    ...
    # Use ProcessPoolExecutor in async context
    if workers < 1:
        workers = cpu_count()
    else:
    ...
    with ProcessPoolExecutor(max_workers=workers) as executor:
        ...
        # Submit all jobs to executor (non-blocking)
        for job in jobs:
            task = loop.run_in_executor(executor, _convert_job, job)
            tasks.append(task)
