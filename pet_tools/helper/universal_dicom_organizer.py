...
def organize_dicom(input_dir, output_root):
    ...
    with tqdm(total=len(dcm_files), desc="Processing DICOMs") as pbar:
        for f in dcm_files:
            try:
                ds = pydicom.dcmread(f, force=False)
            except:
    ...
    print("\n📋 Organizing files...")
    with tqdm(total=len(groups), desc="Organizing series") as pbar:
    ...
    print(f"\n✅ Organization complete. Organized {len(groups)} series into {site_root}\n")
